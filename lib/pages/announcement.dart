...
import 'package:klinix/models/announcement_model.dart';
...
class AnnouncementPage extends StatefulWidget {
  ...
  @override
  AannouncementPageState createState() => AannouncementPageState();
}

class AannouncementPageState extends State<AnnouncementPage> {
  var _list = <AnnouncementModel>[];
  ...
  void fetchList() async {
    ...
    try {
      var res = await http.get(Variables.url + '/getAnnouncement', headers: {
        'Content-Type': 'application/json',
        'Authorization': 'bearer ' + Variables.accessToken
      });
      if (res.statusCode == 200) {
        var decodeList = json.decode(res.body) as List<dynamic>;
        _list = decodeList.map((i) => AnnouncementModel.fromJson(i)).toList();
      }
    } finally {
      setState(() {
        _isLoading = false;
