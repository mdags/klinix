...
class _SearchPageState extends State<SearchPage> {
  ...
  var _searchList = <DoctorsModel>[];
  var _filteredSearchList = <DoctorsModel>[];

  Future<void> getList() async {
    await pr.show();

    var res = await http
        .get(Variables.url + '/getSearchList?lang=' + Variables.lang, headers: {
      'Content-Type': 'application/json',
      'Authorization': 'bearer ' + Variables.accessToken
    });
    if (res.statusCode == 200) {
      var decodeList = json.decode(res.body) as List<dynamic>;
      _searchList = decodeList.map((i) => DoctorsModel.fromJson(i)).toList();
  ...
  search(String value) {
    if (value.isEmpty) {
      setState(() {
        _filteredSearchList = <DoctorsModel>[];
      });
      return;
    }
    setState(() {
  ...
  Future<void> selectObject(DoctorsModel searchValue) async {
    if (searchValue.p2 == 'Kurum') {
      ...
      var res = await http
          .get(Variables.url + '/getDoctorById?id=' + searchValue.p1, headers: {
        'Content-Type': 'application/json',
        'Authorization': 'bearer ' + Variables.accessToken
      });
      if (res.statusCode == 200) {
  ...
  Widget listViewWidget() {
    return ListView.separated(
      ...
      itemBuilder: (context, index) => ListTile(
        leading: _filteredSearchList[index].p2 == 'Kurum'
            ? Image.asset(
                'assets/images/hospital_lead.png',
                width: 32,
                height: 32,
                color: Variables.primaryColor,
              )
            : _filteredSearchList[index].p2 == 'Bölüm'
                ? Image.asset(
        ...
        title: RichText(
          text: TextSpan(
              ...
              text: _filteredSearchList[index].p2 == 'Kurum'
